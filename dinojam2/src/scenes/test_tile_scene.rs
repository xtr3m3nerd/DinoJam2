use crate::asset_management::asset_collections::MapAssets;
    ...
    fn build(&self, app: &mut App) {
        ...
        app.add_system_set(
            ConditionSet::new()
                .run_in_state(AppState::InGame)
                .after("tile_behavior")
                .with_system(highlight_neighbor_label)
                .into(),
        );
    }
}
...
pub fn init_map(mut commands: Commands, assets: Res<MapAssets>, terrain: Res<Terrain>) {
    ...
    let map_size: TilemapSize = UVec2::new(SIZE_X, SIZE_Y).into();
    ...
    for layer in [Layer::Terrain] {
        ...
        commands.entity(map).insert(layer);
    }
...
fn spawn_tile_labels(
    ...
    tile_label_q: Query<&mut Text, With<TileLabel>>,
) {
    for _ in tile_label_q.iter() {
        return;
    }
    ...
    for (map_transform, map_type, grid_size, tilemap_storage, layer) in tilemap_q.iter() {
...
fn hover_highlight_tile_label(
    ...
    for (map_size, grid_size, map_type, tile_storage, map_transform, layer) in tilemap_q.iter() {
        if *layer != Layer::Terrain {
            continue;
        }
...
fn highlight_neighbor_label(
    mut commands: Commands,
    tilemap_query: Query<(&TilemapType, &TilemapSize, &TileStorage, &Layer)>,
    ...
    for (map_type, map_size, tile_storage, layer) in tilemap_query.iter() {
        if *layer != Layer::Terrain {
            continue;
        }
